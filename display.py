...
class KNNModel(Model):
    train_result:py.ndarray
    test_result:py.ndarray
    def __init__(self) -> None:
        super().__init__()


    def train(self, X_train, X_test, y_train, y_test) -> py.ndarray:
    ...
    def test(self, X_test) -> Any:
        self.test_result = classifier.predict(X_test)
...
class ZhunJudger(Judger):
    ...
    def judge(self, train_result,test_result,y_train,y_test) -> Tuple[float,float]:
        train_total = 0
        ...
        train_precision = test_total/y_test.shape[0] #precision是准确率
        return train_precision,train_precision
